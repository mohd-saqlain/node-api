...
const getCustomerDetails = async (req,res) => {
   ...
   try{
    ...
    const coupons = customerCoupons.map(({coupon})=>({coupon_name:coupon.coupon_name,coupon_code:coupon.coupon_code}))
    // console.log(coupons)
...
const createCustomer = async (req,res) => {
  ...
  try{
    ...
    if(customerExists){
        ...
        const customer = new Customer({name,mobile,address,city,dob,merchant_id,customer_key,loyalty_points:100}) 
        ...
        if(customerExists){
            ...
            if(loyalty_points > points){
                const jsonResponse = {
                    status_code:200,
                    response:{
                        points_value:points,authentication:false
                    }
                }
                res.json(jsonResponse);
            }else{
    ...
    try{
        ...
        if(customerExists){
            ...
            if(loyalty_points > points){
                ...
                const updateStatus = await Customer.findOneAndUpdate({mobile:customer_mobile},{loyalty_points:updatedPoints})
                const jsonResponse = {
                    status_code:200,
                    response:{
                        points_value:points
...
const getPOSBillDetails = async (req,res) => {
    ...
    const {number,type,gross_amount,net_amount,order_time,items} = transaction
    try{
        ...
        const bill = new Bill({customer_mobile:mobile,customer_name:name,transaction_number:number,transaction_type:type,gross_amount,net_amount,order_time,items});
...
const getCouponDetails = async (req,res) => {
    ...
    if(userExists){
        const matchedCoupon = await Coupon.findOne({customer_mobile,"coupon.coupon_code":coupon_code})
...
module.exports = {getCustomerDetails,createCustomer,authenticateLoyalityPointRedem,loyaltyPointRedemption,getPOSBillDetails,getCouponDetails,redeemCoupon,addCoupon,getBills}